---
name: scrap-attack-path-analysis
description: Use when already in the attack-path-analysis phase of a security scan or the user explicitly asks to trace a security finding from source to sink and calibrate severity. Do not use as the primary trigger for full PR, commit, branch, patch, or repository scans.
---
...
Use the shared scan artifact path conventions in `../scrap-security-scan/references/scan-artifacts.md`.

## Workflow

1. Load the per-scan threat model path from `../scrap-security-scan/references/scan-artifacts.md` as the repo-specific threat-model source of truth. Start from this along with the potential findings. Both inputs are required for this workflow.
...
- Save a final visible report using the attack-path analysis report path from `../scrap-security-scan/references/scan-artifacts.md`.

## Attack Path Considerations

- A finding should count as a real security issue if a realistic attacker could use it from a reasonable attack surface relevant to the product, especially if it is something that is part of the threat model.
...
For severity calibration criteria, examples, escalation factors, and the acceptance checklist, see `references/severity-policy.md`.
