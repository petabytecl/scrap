---
name: scrap-finding-discovery
description: Use when already in the finding-discovery phase of a security scan or the user explicitly asks to discover candidate security findings in a repository or code change. Do not use as the primary trigger for full PR, commit, branch, patch, or repository scans.
---
...
If the scan target is for a targeted code-diff, follow the procedure in `../scrap-security-scan/references/code-diff-scan.md`.

### Repository-Wide Workflow

If the scan target is repository-wide, follow the procedure in `../scrap-security-scan/references/repository-wide-scan.md` and every required reference it lists.
...
For the full severity taxonomy with examples of critical, high, and non-qualifying findings, see `../scrap-attack-path-analysis/references/severity-policy.md`.
...
- Save a final visible report using the finding discovery report path from `../scrap-security-scan/references/scan-artifacts.md`.
