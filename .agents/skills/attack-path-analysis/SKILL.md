...
Use the shared scan artifact path conventions in `../security-scan/references/scan-artifacts.md`.

## Workflow

1. Load the per-scan threat model path from `../security-scan/references/scan-artifacts.md` as the repo-specific threat-model source of truth. Start from this along with the potential findings. Both inputs are required for this workflow.
...
- Save a final visible report using the attack-path analysis report path from `../security-scan/references/scan-artifacts.md`.
