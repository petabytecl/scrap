# Final Report and Review Directives

Use this guidance when assembling the final security scan markdown report and review directives.
...
## Inline Review Directives

When the environment supports inline code comments, emit one `::code-comment{...}` directive per surviving finding in the final response. The markdown report and review directives should agree on title, priority, file, line range, and core explanation.

For each reportable finding, emit a review directive in this form:
