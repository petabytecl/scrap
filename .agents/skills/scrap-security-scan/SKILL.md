...
1. `scrap-threat-model` (the scrap-threat-model skill)
2. `scrap-finding-discovery` (the scrap-finding-discovery skill)
3. `scrap-validation` (the scrap-validation skill)
4. `scrap-attack-path-analysis` (the scrap-attack-path-analysis skill)
...
2. Run the `scrap-threat-model` skill first.
...
3. Run the `scrap-finding-discovery` skill as the second step, against the resolved diff and using the per-scan threat model as context.
...
4. Run the `scrap-validation` skill as the third step, for each candidate that came out of discovery and, in repository-wide scans, each open, reportable, or deferred seeded/root-control ledger row that still needs closure.
...
5. Run the `scrap-attack-path-analysis` skill as the fourth step, for findings and repository-wide validation closure rows that still need reportability, attack-path, and severity analysis after validation.
...
Assemble the final markdown report using `references/final-report.md`.
