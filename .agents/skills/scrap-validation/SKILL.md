---
name: scrap-validation
description: Use when already in the validation phase of a security scan or the user explicitly asks to determine whether one or more candidate security findings are valid. Do not use as the primary trigger for full PR, commit, branch, patch, or repository scans.
---
...
6. Save any PoC files, inputs, or logs under the validation artifacts path from `../scrap-security-scan/references/scan-artifacts.md`.
...
- Keep validation artifacts and the final visible report in the validation paths from `../scrap-security-scan/references/scan-artifacts.md` so the full scan bundle lives together.
