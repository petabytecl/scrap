...
Establish the repository-scoped threat model at the path defined in `../security-scan/references/scan-artifacts.md`. If this already exists, stop here. If a threat model or clearly authoritative security scan guidance is provided or already exists, persist it unchanged to this file, then stop here.
...
1. Resolve `repo_name`, `security_scans_dir`, and the repository-scoped threat model path using `../security-scan/references/scan-artifacts.md`.
...
- Persist the threat model output to the repository-scoped threat model path from `../security-scan/references/scan-artifacts.md`.
