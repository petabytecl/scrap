...
1. `threat-model` (the threat-model skill)
2. `finding-discovery` (the finding-discovery skill)
3. `validation` (the validation skill)
4. `attack-path-analysis` (the attack-path-analysis skill)
...
2. Run the `threat-model` skill first.
...
3. Run the `finding-discovery` skill as the second step, against the resolved diff and using the per-scan threat model as context.
...
4. Run the `validation` skill as the third step, for each candidate that came out of discovery and, in repository-wide scans, each open, reportable, or deferred seeded/root-control ledger row that still needs closure.
...
5. Run the `attack-path-analysis` skill as the fourth step, for findings and repository-wide validation closure rows that still need reportability, attack-path, and severity analysis after validation.
