---
name: scrap-gh-address-comments
...
When implementing fixes locally, any commits MUST follow Conventional Commits. The canonical type list and format live in `../scrap-git-commit/SKILL.md`. For review-driven changes the typical mapping is:

- `fix(<scope>): ...` — behavior corrections requested in review
- `refactor(<scope>): ...` — non-behavioral cleanup or naming changes
- `docs(<scope>): ...` — documentation or comment updates
- `test(<scope>): ...` — added or adjusted tests in response to feedback
- `perf(<scope>): ...` — performance improvements raised in review

When the same PR addresses multiple unrelated review threads, prefer one commit per logical thread group rather than one giant commit, so each Conventional Commits message stays accurate.
