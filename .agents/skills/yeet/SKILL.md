...
All branches, commits, and PR titles MUST follow the [Conventional Commits](https://www.conventionalcommits.org/) specification. The canonical type list, scope guidance, and breaking-change format live in `../git-commit/SKILL.md` — defer to that skill for the source of truth and do not redefine types here.
...
2. Classify the change and choose the Conventional Commits type.
   - Analyze the diff and pick the single type that best describes it. Consult `../git-commit/SKILL.md` for the canonical type table.
...
- Never invent a Conventional Commits type that is not in the canonical `git-commit` list. When the diff does not fit cleanly, ask the user.
