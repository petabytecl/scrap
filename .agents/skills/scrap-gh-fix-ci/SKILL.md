...
When committing CI fixes locally, follow Conventional Commits. The canonical type list lives in `../scrap-git-commit/SKILL.md`. Most CI fixes map cleanly to one of:

- `ci: ...` — workflow file, action version, or runner config changes
- `build: ...` — build system, lockfile, or dependency resolution fixes
- `test: ...` — test code changes (flaky-test stabilization, fixture updates)
- `fix(<scope>): ...` — application code changes that fix a real bug surfaced by CI
- `chore(deps): ...` — dependency bumps needed to unblock CI

Choose the type that reflects the actual root cause, not the symptom. If CI is failing because a test caught a real bug, the commit is `fix:` not `test:`. If CI is failing because the workflow YAML drifted, it is `ci:` not `fix:`.
