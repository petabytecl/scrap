...
- FR-5 (PLAUSIBLE, medium): `internal/shard/raft_snapshot.go` uses member identity as a proxy for "durable state covers snapshot index": a partial DataDir restore (pebble/blocks older than raft snap/wal) self-matches and silently diverges; identity churn with intact data spuriously fails closed. Deeper fix: record and compare a durable applied watermark in the manifest. **STILL DEFERRED (2026-07-05, re-raised as Codex P2 on PR #453):** the applied index is only an in-memory `index.Index.appliedIndex` (`atomic.Uint64`), never persisted to pebble, so there is no durable watermark to record or compare at restore. A real fix must persist the applied index in the pebble projection (synced on the apply path) and add it to the snapshot manifest — a storage-format change requiring an ADR. Out of scope for the PR #453 review round; needs its own story.
...
- Codex P2 `internal/shard/raft_snapshot.go` — **DEFERRED:** duplicate of FR-5 above; needs durable applied-index persistence + ADR.
