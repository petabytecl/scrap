...
# Story 6.2: Operator Runbooks for SCRAP Failure Domains
...
- [x] Create the SCRAP operator runbook documentation surface. (AC: 1, 2)
...
- [x] Apply a common runbook checklist to every runbook. (AC: 1, 2)
  ...
  - [x] Create `_bmad-output/implementation-artifacts/operator-runbook-evidence.md`.
...
- [x] Keep Story 6.2 inside its release-doc scope. (AC: 1-5)
  - [x] Do not implement new production behavior, new `scrapctl` commands, new admin endpoints, alert/query references, the release evidence bundle, Tier 2/Tier 3 final evidence, real S3/IAM closure, or final SCRAP closure policy.
...
- [x] Run verification and update BMAD tracking. (AC: 1-5)
  ...
  - [x] Run command-surface validation for all documented commands and record it in `_bmad-output/implementation-artifacts/operator-runbook-evidence.md`.
...
- Operator runbooks, alert/query references, incident workflows, and evidence instructions are required unless explicitly de-scoped. Issue `#429` remains a final gate after feature scope is complete. [Source: `_bmad-output/planning-artifacts/prds/prd-scrap-master-2026-06-10/prd.md#FR-16: Major-release evidence and documentation closure`]
...
- mTLS/certificate rotation: document restart or rollout-based rotation only. Hot certificate reload is out of SCRAP; do not imply live reload. [Source: `docs/production-rehearsal.md#Certificate Rotation`]
...
- Restore failures and cold reads: restore uses committed metadata and full-Block verification before serving bytes. Direct Backend streaming and Backend inventory as read authority are out of SCRAP. [Source: `docs/adr/0027-phase-5-restore-first-cold-reads.md`; `_bmad-output/implementation-artifacts/epic-3-backend-durability-cold-read-closure-evidence.md`]
...
Run command-surface validation and record results in `_bmad-output/implementation-artifacts/operator-runbook-evidence.md`:
...
rg -n --pcre2 "$secret_shape" docs/runbooks _bmad-output/implementation-artifacts/6-2-operator-runbooks-for-failure-domains.md _bmad-output/implementation-artifacts/operator-runbook-evidence.md
rg -n --pcre2 "$authority_shape" docs/runbooks _bmad-output/implementation-artifacts/6-2-operator-runbooks-for-failure-domains.md _bmad-output/implementation-artifacts/operator-runbook-evidence.md
rg -n --pcre2 "$identity_shape" docs/runbooks _bmad-output/implementation-artifacts/6-2-operator-runbooks-for-failure-domains.md _bmad-output/implementation-artifacts/operator-runbook-evidence.md
```
...
- Story file: `_bmad-output/implementation-artifacts/6-2-operator-runbooks-for-failure-domains.md`.
- Expected durable docs: `docs/runbooks/*.md`.
- Expected evidence artifact: `_bmad-output/implementation-artifacts/operator-runbook-evidence.md`.
...
- `_bmad-output/planning-artifacts/prds/prd-scrap-master-2026-06-10/prd.md` - FR-16 and release evidence requirements.
...
- `docs/scope-reconciliation.md` - final release gate order and issue `#429` context.
- `docs/adr/0025-content-quarantine-admin-surface.md` - Content Quarantine operator/admin surface.
- `docs/adr/0026-multi-shard-release-boundary.md` - multi-Shard release boundary.
- `docs/adr/0027-phase-5-restore-first-cold-reads.md` - restore-first cold-read boundary.
- `_bmad-output/implementation-artifacts/6-1-release-evidence-matrix.md` - previous Story 6.1 record and review findings.
- `_bmad-output/implementation-artifacts/release-evidence-matrix.md` - release evidence row schema and current gap classification.
...
- 2026-06-12T18:30:43-04:00 - Created `docs/runbooks/` with ten SCRAP operator runbooks plus index, created `operator-runbook-evidence.md`, validated commands, classified redaction/authority scan matches, and passed all required gates.
...
- Created the durable SCRAP operator runbook set under `docs/runbooks/`.
- Added `_bmad-output/implementation-artifacts/operator-runbook-evidence.md` with per-runbook checklist coverage, command validation, authority review, redaction review, false-positive classification, and remaining release scope.
...
- Verification passed: `git diff --check`, `make proto-check`, `scripts/check-e2e-gates.sh`, command-surface validation, redaction/authority scans, and `env GOCACHE=/tmp/scrap-go-build make check`.
...
- `_bmad-output/implementation-artifacts/6-2-operator-runbooks-for-failure-domains.md`
- `_bmad-output/implementation-artifacts/sprint-status.yaml`
- `_bmad-output/implementation-artifacts/operator-runbook-evidence.md`
...
- 2026-06-12 - Created Story 6.2 context for SCRAP operator runbooks.
- 2026-06-12 - Started Story 6.2 implementation.
- 2026-06-12 - Implemented SCRAP operator runbooks and moved Story 6.2 to review.
