...
| DR-2 | DR drill evidence | DR drill output records measured recovery evidence without making an unapproved formal RTO/RPO promise. | `TestRunQueuedOperationsOnceDryRunDROperationReportsReadiness`, `TestRunQueuedOperationsOnceDRDrillRestoresScratchMetadata`, DR drill failure tests, and the DR rebuild drill runbook. | #48, #88 | Blocked until human decisions and drill artifact exist. |
