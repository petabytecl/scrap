...
| `SCRAP_OPERATOR_READINESS` | Operators can see, audit, stop, retry, and recover dangerous actions using admin API and `scrapctl` workflows. | Admin operation, audit, authorization, durable operation tests, and `docs/storage-gateway-dashboard-alert-contract.md`. | #47, #88 | Blocked until runbooks and DR drill evidence exist. |
...
| OPS-1 | Operator control path | Dangerous actions are typed, durable, authorized, audited, and idempotent through the admin API and `scrapctl`, not scripts. | Admin plan/start tests, audit tests, authz denial tests, durable operation tests, release aggregation, and the dashboard/alert contract. | #47 | Blocked until runbooks exist. |
