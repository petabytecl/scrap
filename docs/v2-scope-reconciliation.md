...
This document reconciles the current V2 repo, accepted ADRs, BMAD planning
artifacts, GitHub issue state, and older project documentation after the
release rule was clarified:

> V2 is the major release line. There are no intermediate releases. V2 is not
> release-ready until all required V2 features are complete.
...
1. `CONTEXT.md` and accepted ADRs define the durable V2 contract unless later
   ADRs supersede them.
2. V1 material is reference input, not a V2 requirement by itself. V2 only
   inherits old V1 work when it is re-derived into V2 docs, ADRs, PRDs, or
   issues.
...
- Branch checked: `v2`.
- GitHub milestone `storage-gateway-v2`: `0` open, `110` closed.
...
claim all required V2 features are complete.

## Scope Matrix

| Area | Source | V2 classification | Current state | Required next action |
| --- | --- | --- | --- | --- |
| Core Document API, immutable Document identity, Transaction grouping, Block/Frame format, checksum semantics | `CONTEXT.md`, ADR 0001-0005, Phase 1/2 issues | Required | Implemented by earlier V2 slices | Keep covered by final regression evidence |
| Multi-voter Raft, peer replication, Member identity, fail-closed Projection Resolution | `CONTEXT.md`, ADR 0014, Phase 2 issues | Required | Implemented by earlier V2 slices | Keep covered by final regression/e2e evidence |
...
| OpenTelemetry evidence plane | ADR 0012, ADR 0013, issues #312-#318 | Required | Code and evidence stack exist; focused tests pass | Produce final V2 evidence matrix and current evidence bundle |
| Prod-like Kind/Cilium verification and `scrapctl` gates | ADR 0015, issue #350 | Required | Implemented as a validation lane | Re-run as part of final V2 evidence, not as release-only polish |
...
| Production security mode, mTLS, authz, peer scope, audit, rate limits | ADR 0019, ADR 0024, issues #399, #401-#404, #430-#434 | Required | Implemented by closed Phase 4.5/security follow-ups | Keep covered by final V2 security evidence |
| OpenBao Transit envelope encryption and durable rewrap | ADR 0020, ADR 0021, ADR 0023, issues #400, #405-#407 | Required | Implemented by closed Phase 4.5 slices | Keep covered by final V2 security/e2e evidence |
...
| Content Quarantine admin operations | ADR 0008 amended by ADR 0025 | Required through existing admin HTTP plus `scrapctl` | Current admin surface is HTTP-oriented; ADR 0025 keeps that shape for V2 | Include in Content Scanner/Quarantine backlog |
| Phase 5 cold-only reads / all-local-copy eviction | `CONTEXT.md`, ADR 0016, ADR 0020, ADR 0027 | Required as restore-first cold reads | Not implemented; ADR 0027 rejects direct Backend streaming for V2 | Create Phase 5 epics/stories |
| Direct Backend ciphertext streaming | ADR 0016, ADR 0020, ADR 0027 | Out of V2 unless re-chartered | Rejected for Phase 4 and out of V2 by ADR 0027 | Do not include in V2 backlog |
...
| Alert definitions, incident workflows, operator runbooks | ADR 0012, production-readiness docs | Required for a production-ready major release unless explicitly deferred | ADR 0012 deferred them out of the telemetry decision, not necessarily out of V2 | Create V2 operator docs/runbook backlog |
...
The audited docs do not currently make these required for V2 release:
...
If any of these are intended V2 release features, they need explicit V2 PRD or
...
1. ADR 0008 Content Scanner / Content Quarantine is required for V2. ADR 0025
   amends the admin surface to existing admin HTTP plus `scrapctl`.
2. Multi-Shard startup/routing is required for V2 by ADR 0026.
3. Phase 5 read shape is restore-first cold reads by ADR 0027. Direct Backend
   ciphertext streaming is out of V2 unless re-chartered.
...
1. Regenerate V2 epics/stories from the master PRD, master architecture, ADR
...
V2 is not feature complete.

The current repository appears substantially complete for Phases 1 through 4.5
as previously scoped. The remaining required V2 work is not just release
...
final evidence all need explicit backlog treatment before V2 can be called
release-ready.
