...
// crashed pod), and an optional intercept can observe or drop messages.
...
func newFaultableTransport() *faultableTransport {
	return &faultableTransport{nodes: make(map[uint64]*Node)}
}
...
// leadership changes.
func (c *catchupCluster) proposeOnce(t *testing.T, ctx context.Context, seq int) {
	t.Helper()
